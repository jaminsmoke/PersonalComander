...
Rutas Identity que Commander llama: derivadas de `IdentityCliente.kt` (`Rutas.*`; sin snapshot). Rutas Bar que Commander llama: [`bar-contract-paths.txt`](bar-contract-paths.txt). El job CI `Family contracts` las contrasta con Identity `main` (OpenAPI camareros y negocio) y Bar `main` (`BarLanModule.kt` + `openapi-lan.json`), verificando también `Authorization` (Bearer); el informe queda en el summary del job. Aún no es check requerido del ruleset.
